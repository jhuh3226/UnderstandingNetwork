...
//int port = 8883;
int port = 8883;
char topic[] = "jReadings";
const char willTopic[] = "jReadings/will";
String clientID = "jRead";
...
void loop() {
  ...
  if (!mqttClient.connected()) {
    ...
    if (abs(rtc.getMinutes() - lastSendTime) >= sendInterval) {
      ...
      readingCount++;
      // added this part to see if the value changes
      luxIncrease++;
      ctIncrease++;
...
void readSensor() {
  ...
  body["timeStamp"] = rtc.getEpoch();
  body["lux"] = 100 + luxIncrease;
  body["ct"] = 200 + ctIncrease;
  body["uptime"] = rtc.getEpoch() - startTime;
  body["readingCount"] = readingCount;
}
